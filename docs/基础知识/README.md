---
sidebar: false
title: 基础知识
---
## 该章节包含以下内容
#### [HTTP](./HTTP)
    
- [HTTP、HTTPS代理分析及原理](./HTTP/HTTP、HTTPS代理分析及原理.md)

  
- [HTTP缓存机制](./HTTP/HTTP缓存机制.md)

  
- [常见的状态码](./HTTP/常见的状态码.md)

  
- [输入网址后发生什么](./HTTP/输入网址后发生什么.md)

  
#### [JavaScript](./JavaScript)
    
- [AJAX](./JavaScript/AJAX.md)

  
- [Canvas](./JavaScript/Canvas.md)

  
- [ES6](./JavaScript/ES6.md)

  
- [Promise](./JavaScript/Promise.md)

  
- [this](./JavaScript/this.md)

  
- [事件循环](./JavaScript/事件循环.md)

  
- [作用域与闭包](./JavaScript/作用域与闭包.md)

  
- [原型](./JavaScript/原型.md)

  
- [对象](./JavaScript/对象.md)

  
- [异步编程](./JavaScript/异步编程.md)

  
- [正则表达式](./JavaScript/正则表达式.md)

  
- [类型与值](./JavaScript/类型与值.md)

  
- [高阶函数](./JavaScript/高阶函数.md)

  
#### [TypeScript](./TypeScript)
    
- [Typescript基础](./TypeScript/Typescript基础.md)

  
- [泛型](./TypeScript/泛型.md)

  
#### [vue3](./vue3)
    
- [vue3基础](./vue3/vue3基础.md)

  
- [vue3新特性](./vue3/vue3新特性.md)

  
- [vue3路由](./vue3/vue3路由.md)

  
#### [其他](./其他)
    
- [性能优化](./其他/性能优化.md)

  
- [迭代](./其他/迭代.md)

  
- [面向对象](./其他/面向对象.md)

  
#### [后端](./后端)
    
- [Koa](./后端/Koa.md)

  
- [Nodejs](./后端/Nodejs.md)

  
- [即时通信](./后端/即时通信.md)

  
- [后端项目搭建](./后端/后端项目搭建.md)

  
- [模板引擎](./后端/模板引擎.md)

  
#### [浏览器](./浏览器)
    
- [事件机制](./浏览器/事件机制.md)

  
- [客户端存储](./浏览器/客户端存储.md)

  
- [渲染原理](./浏览器/渲染原理.md)

  
- [缓存机制](./浏览器/缓存机制.md)

  
- [跨域问题](./浏览器/跨域问题.md)

  
#### [计算机网络](./计算机网络)
    
- [TCP](./计算机网络/TCP.md)

  
- [UDP](./计算机网络/UDP.md)

  
- [localhost和127.0.0.1的区别](./计算机网络/localhost和127.0.0.1的区别.md)

  
- [当ping一个IP地址会发生什么](./计算机网络/当ping一个IP地址会发生什么.md)

  